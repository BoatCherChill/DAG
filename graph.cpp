...
vector<vector<int>> Graph::adjacencyMatrix() const {
    ...
    map<int, int> idToIndex;                             // Отображение ID узла -> индекс в векторе
    for (int i = 0; i < n; i++) idToIndex[nodes[i].nodeID] = i;
    for (int i = 0; i < n; i++) {
        for (int prevId : nodes[i].prev_nodes) {          // Для каждого предка узла i
            int from = idToIndex[prevId];
            int to = i;
            if (from >= 0 && to >= 0) adj[from][to] = 1; // Ребро from -> to
        }
    }
    return adj;
}
...
bool Graph::isDAG() const {
    ...
    for (int i = 0; i < n; i++) if (!visited[i] && hasCycle(i, visited, recStack)) return false;
    return true;
}
...
void Graph::addNode(int x, int y, NodeType type) {
    ...
    newNode.coordinates = { x, y };                      // Сохраняем координаты
    newNode.type = type;
...
Node* Graph::findNodeById(int id) {
    for (auto& node : nodes) if (node.nodeID == id) return &node; // Поиск по ID
    return nullptr;
}
...
void Graph::addRelation(int fromId, int toId) {
    ...
    if (!toNode) return;
    for (int prev : toNode->prev_nodes) if (prev == fromId) return; // Связь уже существует
    toNode->prev_nodes.push_back(fromId);                 // Добавляем связь
}
...
void Graph::deleteRelation(int fromId, int toId) {
    ...
    if (it != prevNodes.end()) prevNodes.erase(it);      // Удаляем связь
}
...
string Graph::sum(const string& data) const {
    ...
    double num;
    while (ss >> num) result += num;                     // Суммирование всех чисел
...
string Graph::average(const string& data) const {
    ...
    stringstream ss(data);
    double num;
    while (ss >> num) { sumVal += num; count++; }        // Суммирование и подсчет
    if (count == 0) return "0";
...
string Graph::median(const string& data) const {
    ...
    stringstream ss(data);
    double num;
    while (ss >> num) nums.push_back(num);               // Сбор всех чисел
    if (nums.empty()) return "0";
    sort(nums.begin(), nums.end());                      // Сортировка
    size_t n = nums.size();
    double median;
    if (n % 2 == 0) median = (nums[n/2 - 1] + nums[n/2]) / 2.0; // Четное количество
    else median = nums[n/2];                             // Нечетное количество
...
void Graph::execute() {
    if (!isDAG()) return;                                // Проверка на ацикличность
    ...
    for (int nodeId : order) {
        ...
        // Выполнение операции в зависимости от типа узла
        switch (node->type) {
            case NodeType::INPUT:
